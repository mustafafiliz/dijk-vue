    ...
    <div class="bg-white rounded-3xl shadow-lg w-11/12 md:w-1/3">
      <div class="p-4">
        ...
        <div class="mb-2 text-xs text-gray-700">
          ...
          <div class="mt-1">
            {{ dateOnly(permitStartDate) }} {{ permitStartTime }} - {{ dateOnly(permitEndDate) }}
            {{ permitEndTime }}
          </div>
        </div>
        <p class="mb-2 text-base text-blue-700">
          <strong class="text-gray-900 text-sm">Toplam:</strong> <b>{{ remainingDays }} gün</b>
        </p>
      </div>
      <div class="flex">
        <Button
        ...
        <Button
          v-else
          @click="$router.push('/dashboard/permission-request/list')"
          class="!rounded-t-none bg-green-600 !w-full !py-3"
          >İzin Oluşturuldu</Button
        >
      </div>
    </div>
  </div>
