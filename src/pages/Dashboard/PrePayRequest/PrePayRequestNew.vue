...
const price = ref('')
const pre_pay_type_id = ref('')
const pre_pay_options = ref([])
...
const selectedCurrency = computed(() => {
  const selected = pre_pay_options.value.find((option) => option._id === pre_pay_type_id.value)
  return selected?.currency || ''
})

const selectedPrePayType = computed(() => {
  return pre_pay_options.value.find((option) => option._id === pre_pay_type_id.value)?.title || ''
})

const isFormValid = computed(() => {
  return pre_pay_type_id.value !== '' && date.value !== '' && price.value !== ''
})

const getPrePayOptions = async () => {
  try {
    const { data } = await axios.get('/pre-pay-types')
    pre_pay_options.value = data
    pre_pay_type_id.value = data[0]._id
  } catch (error) {
...
const onSubmit = async () => {
  try {
    ...
    await axios.post('/pre-pays', {
      pre_pay_type_id: pre_pay_type_id.value,
  ...
  >
    <div class="relative flex flex-col p-4 pb-20 flex-1 max-w-5xl">
      ...
      <div class="flex flex-col gap-4 pt-8">
        <div>
          <div class="font-semibold mb-[10px]">Avans Türü <span class="text-red-500">*</span></div>
          <div class="bg-white px-2 rounded-3xl h-[45.5px] flex">
            <select
              v-model="pre_pay_type_id"
              class="w-full rounded-3xl text-[12px] lg:text-base h-full px-1 outline-none"
            >
              <option value="">Seçiniz</option>
              <option v-for="option in pre_pay_options" :key="option._id" :value="option._id">
                {{ option.title }}
              </option>
            </select>
          </div>
    ...
    <div class="bg-white rounded-3xl shadow-lg w-11/12 md:w-1/3">
      <div class="p-4">
        <div class="flex items-center mb-4">
          ...
          <h2 class="text-lg font-semibold ml-2">Avans Talebi</h2>
        </div>
        <p class="mb-4 text-sm text-gray-700">
          Girdiğiniz bilgilere göre avans talebi oluşturulacak.
        </p>
        <div class="mb-2 text-xs text-gray-700">
          <strong class="text-gray-900 text-sm">Avans Türü:</strong>
