...
<template>
  <div>
    <div class="flex items-center gap-x-4 bg-white rounded-2xl py-3 px-4">
      <img
        class="border-night-sky border-[3px] w-12 h-12 box-content object-cover rounded-full"
        :src="application.userImage"
        alt=""
      />

      <div class="text-squant">
        <div class="text-14 font-semibold">
          <div class="text-black">
            {{ application.userName }}
          </div>
          <div>{{ application.type }} istiyor.</div>
        </div>

        <div class="text-12 font-medium">
          <div>{{ application.startDate }} - {{ application.endDate }}</div>
          <div>Toplam: <span class="text-gentian-flower font-semibold"> 1 Gün </span></div>
        </div>
      </div>
    </div>
    <div class="flex mt-2 gap-x-2">
      <template v-if="isApproved === null">
        ...
        <button
          class="flex-1 flex items-center justify-center gap-x-2 bg-white rounded-[10px] p-2 text-gamora font-medium text-14"
          type="button"
